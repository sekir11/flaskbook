...
def test_upload_signup_post(client):
    signup(client, "adim", "flaskbook@example.com", "password")
    rv = upload_image(client, "detector/testdata/4.png")
...
def test_detect_no_user_image(client):
    signup(client, "admin", "flaskbook@example.com", "password")
    upload_image(client, "detector/testdata/4.png")
    # 存在しないIDを指定する
...
def test_detect(client):
    ...
    assert user_image.image_path in rv.data.decode()
    assert "bicycle" in rv.data.decode()
...
def test_detect_search(client):
    ...
    # bicycleワードで検索する
    rv = client.get("/images/search?search=bicycle")

    # bicycleタグの画像があることを確認する
    assert user_image.image_path in rv.data.decode()
    ...
    # bicycleタグの画像がないことを確認する
    assert user_image.image_path not in rv.data.decode()

    # bicycleタグがないことを確認する
    assert "bicycle" not in rv.data.decode()
