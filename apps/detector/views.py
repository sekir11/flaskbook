...
def make_color(lables):
    # 枠線の色をランダムに決定
    colors = [[random.randint(0, 255) for _ in range(3)] for _ in lables]
...
def draw_texts(result_image, line, c1, cv2, color, lables, label):
    # 検知したテキストラベルを画像に追記
    display_txt = f"{lables[label]}"
    font = max(line - 1, 1)
